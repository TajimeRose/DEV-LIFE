...
export default async function Page({ params, searchParams }: { params: Promise<{ projectId: string }>; searchParams: Promise<{ create?: string }> }) {
  ...
  return <><PageHeader eyebrow="FLOWCHARTS" title={project.name} description="ออกแบบกระบวนการด้วยแผนภาพที่แก้ไขและบันทึกอัตโนมัติ" /><FlowchartList projectId={projectId} flowcharts={data ?? []} initialCreate={query.create === "1"} /></>;
}
