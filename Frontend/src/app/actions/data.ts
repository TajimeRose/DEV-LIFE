...
async function accessibleProject(projectId: string, actionContext?: Awaited<ReturnType<typeof context>>) {
  const { supabase, user } = actionContext ?? await context();
  await authorizeProjectAccess(supabase, user.id, uuid.parse(projectId));
  return { supabase, user };
}
...
async function activity(projectId: string, action: string, entityType: string, entityId: string, metadata: Json) {
  const actionContext = await accessibleProject(projectId);
...
export async function createTask(input: unknown) { const value = taskInput.parse(input), actionContext = await accessibleProject(value.project_id), { supabase } = actionContext; const { data, error } = await supabase.from("tasks").insert({ id: crypto.randomUUID(), ...value }).select().single(); if (error) throw new Error(error.message); try { await insertActivity(actionContext, data.project_id, "created", "task", data.id, { title: data.title }); } catch {} return data; }
...
export async function updateTask(id: string, input: unknown, changeSummary = "Task updated") { const taskId = uuid.parse(id), value = taskInput.omit({ project_id: true }).partial().refine(v => Object.keys(v).length > 0).parse(input); const actionContext = await context(), { supabase } = actionContext; const { data: old, error: readError } = await supabase.from("tasks").select("*").eq("id", taskId).single(); if (readError || !old) throw new Error("Task not found"); const ownedContext = await accessibleProject(old.project_id, actionContext); const { data, error } = await supabase.from("tasks").update(value).eq("id", taskId).eq("project_id", old.project_id).select().single(); if (error) throw new Error(error.message); await Promise.all([insertVersion(ownedContext, versionInput.parse({ project_id: old.project_id, entity_type: "task", entity_id: taskId, old_data: old, new_data: data, change_summary: changeSummary })), insertActivity(ownedContext, old.project_id, data.status === "done" && old.status !== "done" ? "completed" : "updated", "task", taskId, value)]); return data; }
export async function deleteTask(id: string) { const taskId = uuid.parse(id), { supabase } = await context(); const { data, error } = await supabase.from("tasks").select("*").eq("id", taskId).single(); if (error || !data) throw new Error("Task not found"); await accessibleProject(data.project_id); await activity(data.project_id, "deleted", "task", taskId, { title: data.title }); const result = await supabase.from("tasks").delete().eq("id", taskId).eq("project_id", data.project_id); if (result.error) throw new Error(result.error.message); }

export async function createNote(input: unknown) { const value = noteInput.parse(input), actionContext = await accessibleProject(value.project_id), { supabase } = actionContext; const { data, error } = await supabase.from("notes").insert({ id: crypto.randomUUID(), ...value }).select().single(); if (error) throw new Error(error.message); try { await insertActivity(actionContext, data.project_id, "created", "note", data.id, { title: data.title }); } catch {} return data; }
...
export async function createVersion(input: unknown) { const value = versionInput.parse(input), actionContext = await accessibleProject(value.project_id); return insertVersion(actionContext, value); }
