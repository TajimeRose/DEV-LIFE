...
async function accessibleProject(projectId: string, actionContext?: Awaited<ReturnType<typeof context>>) {
  const { supabase, user } = actionContext ?? await context();
  const access = await authorizeProjectAccess(supabase, user.id, uuid.parse(projectId));
  return { supabase, user, access };
}
async function editableProject(projectId: string, actionContext?: Awaited<ReturnType<typeof context>>) {
  const result = await accessibleProject(projectId, actionContext);
  if (!canEditProject(result.access.role)) throw new Error("คุณมีสิทธิ์อ่านอย่างเดียวในโปรเจกต์นี้");
  return result;
}
...
async function activity(projectId: string, action: string, entityType: string, entityId: string, metadata: Json) {
  const actionContext = await editableProject(projectId);
...
export async function getProjects() { const { supabase, user } = await context(); const [owned, memberships] = await Promise.all([supabase.from("projects").select("*").eq("user_id", user.id).order("created_at", { ascending: false }), supabase.from("project_members").select("project_id").eq("user_id", user.id)]); if (owned.error) throw new Error(owned.error.message); if (memberships.error) throw new Error(memberships.error.message); const ids = [...new Set((memberships.data ?? []).map(item => item.project_id))]; const shared = ids.length ? await supabase.from("projects").select("*").in("id", ids) : { data: [], error: null }; if (shared.error) throw new Error(shared.error.message); return [...(owned.data ?? []), ...(shared.data ?? [])].filter((project, index, items) => items.findIndex(item => item.id === project.id) === index).sort((left, right) => new Date(right.created_at ?? 0).getTime() - new Date(left.created_at ?? 0).getTime()); }
...
export async function createTask(input: unknown) { const value = taskInput.parse(input), actionContext = await editableProject(value.project_id), { supabase } = actionContext; const { data, error } = await supabase.from("tasks").insert({ id: crypto.randomUUID(), ...value }).select().single(); if (error) throw new Error(error.message); try { await insertActivity(actionContext, data.project_id, "created", "task", data.id, { title: data.title }); } catch {} return data; }
export async function getTasks(projectId: string) { const id = uuid.parse(projectId), { supabase } = await accessibleProject(id); const { data, error } = await supabase.from("tasks").select("*").eq("project_id", id).order("created_at", { ascending: false }); if (error) throw new Error(error.message); return data; }
export async function updateTask(id: string, input: unknown, changeSummary = "Task updated") { const taskId = uuid.parse(id), value = taskInput.omit({ project_id: true }).partial().refine(v => Object.keys(v).length > 0).parse(input); const actionContext = await context(), { supabase } = actionContext; const { data: old, error: readError } = await supabase.from("tasks").select("*").eq("id", taskId).single(); if (readError || !old) throw new Error("Task not found"); const ownedContext = await editableProject(old.project_id, actionContext); const { data, error } = await supabase.from("tasks").update(value).eq("id", taskId).eq("project_id", old.project_id).select().single(); if (error) throw new Error(error.message); await Promise.all([insertVersion(ownedContext, versionInput.parse({ project_id: old.project_id, entity_type: "task", entity_id: taskId, old_data: old, new_data: data, change_summary: changeSummary })), insertActivity(ownedContext, old.project_id, data.status === "done" && old.status !== "done" ? "completed" : "updated", "task", taskId, value)]); return data; }
export async function deleteTask(id: string) { const taskId = uuid.parse(id), actionContext = await context(), { supabase } = actionContext; const { data, error } = await supabase.from("tasks").select("*").eq("id", taskId).single(); if (error || !data) throw new Error("Task not found"); await editableProject(data.project_id, actionContext); await activity(data.project_id, "deleted", "task", taskId, { title: data.title }); const result = await supabase.from("tasks").delete().eq("id", taskId).eq("project_id", data.project_id); if (result.error) throw new Error(result.error.message); }

export async function createNote(input: unknown) { const value = noteInput.parse(input), actionContext = await editableProject(value.project_id), { supabase } = actionContext; const { data, error } = await supabase.from("notes").insert({ id: crypto.randomUUID(), ...value }).select().single(); if (error) throw new Error(error.message); try { await insertActivity(actionContext, data.project_id, "created", "note", data.id, { title: data.title }); } catch {} return data; }
export async function getNotes(projectId: string) { const id = uuid.parse(projectId), { supabase } = await accessibleProject(id); const { data, error } = await supabase.from("notes").select("*").eq("project_id", id).order("created_at", { ascending: false }); if (error) throw new Error(error.message); return data; }
export async function updateNote(id: string, input: unknown, changeSummary = "Note updated") { const noteId = uuid.parse(id), value = noteInput.omit({ project_id: true }).partial().refine(v => Object.keys(v).length > 0).parse(input); const actionContext = await context(), { supabase } = actionContext; const { data: old, error: readError } = await supabase.from("notes").select("*").eq("id", noteId).single(); if (readError || !old) throw new Error("Note not found"); const ownedContext = await editableProject(old.project_id, actionContext); const { data, error } = await supabase.from("notes").update(value).eq("id", noteId).eq("project_id", old.project_id).select().single(); if (error) throw new Error(error.message); await Promise.allSettled([insertVersion(ownedContext, versionInput.parse({ project_id: old.project_id, entity_type: "note", entity_id: noteId, old_data: old, new_data: data, change_summary: changeSummary })), insertActivity(ownedContext, old.project_id, "updated", "note", noteId, value)]); return data; }

export async function getActivityTimeline(projectId: string) { const id = uuid.parse(projectId), { supabase } = await accessibleProject(id); const { data, error } = await supabase.from("activities").select("*").eq("project_id", id).order("created_at", { ascending: false }); if (error) throw new Error(error.message); return data; }
export async function createVersion(input: unknown) { const value = versionInput.parse(input), actionContext = await editableProject(value.project_id); return insertVersion(actionContext, value); }
export async function getVersions(projectId: string, entityType?: "task" | "note", entityId?: string) { const id = uuid.parse(projectId), { supabase } = await accessibleProject(id); let query = supabase.from("versions").select("*").eq("project_id", id); if (entityType) query = query.eq("entity_type", entityType); if (entityId) query = query.eq("entity_id", uuid.parse(entityId)); const { data, error } = await query.order("created_at", { ascending: false }); if (error) throw new Error(error.message); return data; }
export async function restoreVersion(id: string) { const versionId = uuid.parse(id), { supabase } = await context(); const { data: version, error } = await supabase.from("versions").select("*").eq("id", versionId).single(); if (error || !version || !version.project_id || !version.entity_id) throw new Error("Version not found"); await accessibleProject(version.project_id); if (version.entity_type === "task") return updateTask(version.entity_id, version.old_data as Partial<Task>, `Restored version ${version.id}`); return updateNote(version.entity_id, version.old_data as Partial<Note>, `Restored version ${version.id}`); }
