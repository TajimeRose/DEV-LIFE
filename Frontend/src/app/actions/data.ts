...
export async function createTask(input: unknown) { const value = taskInput.parse(input), actionContext = await ownProject(value.project_id), { supabase } = actionContext; const { data, error } = await supabase.from("tasks").insert({ id: crypto.randomUUID(), ...value }).select().single(); if (error) throw new Error(error.message); await insertActivity(actionContext, data.project_id, "created", "task", data.id, { title: data.title }); return data; }
...
export async function createNote(input: unknown) { const value = noteInput.parse(input), actionContext = await ownProject(value.project_id), { supabase } = actionContext; const { data, error } = await supabase.from("notes").insert({ id: crypto.randomUUID(), ...value }).select().single(); if (error) throw new Error(error.message); await insertActivity(actionContext, data.project_id, "created", "note", data.id, { title: data.title }); return data; }
