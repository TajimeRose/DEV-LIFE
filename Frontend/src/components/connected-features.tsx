...
export function ConnectedActivity({ project, activities, repositoryActivities = [] }: { project: Project; activities: Activity[]; repositoryActivities?: ProjectActivity[] }) {
  ...
  return <><div className="heading"><div><small>ACTIVITY</small><h1>{project.name}</h1><p>ประวัติการเปลี่ยนแปลงจาก Supabase</p></div></div><section className="panel feature-panel"><div className="timeline">{items.map(item => <div key={item.id}><span /><section><b>{item.title}</b><small>{item.date ? new Date(item.date).toLocaleString("th-TH") : "ไม่ทราบเวลา"}</small></section></div>)}{!items.length && <p className="empty-copy">ยังไม่มีกิจกรรม</p>}</div></section></>;
}
