...
export function FlowchartList({ projectId, flowcharts, initialCreate = false }: { projectId: string; flowcharts: FlowchartRecord[]; initialCreate?: boolean }) {
  ...
  return <><div className="flow-list-actions"><Button variant="primary" onClick={() => setCreateOpen(true)}>สร้างแผนผัง</Button></div>{flowcharts.length ? <div className="flowchart-card-grid">{flowcharts.map(flowchart => <Card as="article" className="flowchart-card" key={flowchart.id}><div><span>{flowchart.nodes.length} จุด</span><h2>{flowchart.name}</h2><p>{flowchart.description || "ไม่มีรายละเอียด"}</p><small>แก้ไขล่าสุด {new Date(flowchart.updated_at).toLocaleString("th-TH")}</small></div><div className="flowchart-card-actions"><Button variant="primary" onClick={() => setSelected(flowchart)}>เปิด</Button><Button onClick={() => setRenameTarget(flowchart)}>เปลี่ยนชื่อ</Button><Button disabled={pending} onClick={() => mutate(() => duplicateFlowchart(projectId, flowchart.id), "ทำสำเนาแล้ว")}>ทำสำเนา</Button><Button variant="danger" disabled={pending} onClick={() => window.confirm(`ลบ “${flowchart.name}” หรือไม่?`) && mutate(() => deleteFlowchart(projectId, flowchart.id), "ลบแผนผังแล้ว")}>ลบ</Button></div></Card>)}</div> : <EmptyState title="ยังไม่มีแผนผัง" description="สร้างแผนผังแรกแล้วเริ่มออกแบบจากพื้นที่ว่างได้ทันที" action={<Button variant="primary" onClick={() => setCreateOpen(true)}>สร้างแผนผัง</Button>} />}
