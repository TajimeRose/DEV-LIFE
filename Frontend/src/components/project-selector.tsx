...
function ProjectButton({ project }: { project: ProjectSummary }) {
  ...
  return <button type="submit" disabled={pending} aria-label={`เปิดโปรเจกต์ ${project.name}`}>
    ...
    <span className={pending ? "project-open-state" : ""}>
      {pending ? <><span className="ui-spinner" aria-hidden="true" /> กำลังเปิด</> : "เปิดโปรเจกต์"}
    </span>
  </button>;
}
...
export function ProjectSelector({ projects, displayName }: { projects: ProjectSummary[]; displayName: string }) {
  ...
  return <main className="project-select-page"><section className="project-select-shell">
    <header className="project-select-header"><div className="project-select-brand"><Image src="/Logo.png" alt="Dev Life" width={48} height={48} priority /><div><b>DEV LIFE</b><small>พื้นที่ทำงานของนักพัฒนา</small></div></div><form action={logout}><Button type="submit">ออกจากระบบ</Button></form></header>
    <div className="project-select-intro"><small>สวัสดี {displayName}</small><div><h1>เลือกโปรเจกต์</h1>{projects.length > 0 && <span>{projects.length} โปรเจกต์</span>}</div><p>งาน โน้ต และกิจกรรมจะแสดงตามโปรเจกต์ที่คุณเลือก</p></div>
