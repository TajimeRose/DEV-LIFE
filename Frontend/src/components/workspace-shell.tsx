...
import { Button, Dropdown, PageHeader, ToastProvider } from "@/components/ui";
import { navigation } from "@/lib/workspace";

export function WorkspaceShell({ children, email, displayName, projectName }: { children: React.ReactNode; email?: string; displayName?: string; projectName?: string }) {
  ...
  return <ToastProvider><main className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
    <aside id="workspace-navigation" className={`sidebar ${mobile ? "sidebar-open" : ""}`}><div className="sidebar-head"><Link className="brand" href="/dashboard"><Image src="/Logo.png" alt="Dev Life" width={42} height={42} priority /><div><b>DEV LIFE</b><small>Project cockpit</small></div></Link><button className="collapse-button" aria-label={collapsed ? "ขยายแถบเมนู" : "พับแถบเมนู"} aria-expanded={!collapsed} aria-controls="workspace-navigation" onClick={() => mobile ? setMobile(false) : toggleCollapsed()}><i /><i /><i /></button></div><div className="project-switcher"><small>โปรเจกต์ที่กำลังเปิด</small><b>{projectName || "ยังไม่มีโปรเจกต์"}</b><span><i /> งานและข้อมูลในหน้าต่างนี้อ้างอิงจากโปรเจกต์นี้</span></div><nav aria-label="เมนูหลัก">{groups.map(group => <section key={group}><p>{group}</p>{navigation.filter(item => item.group === group).map(item => <Link key={item.href} className={pathname === item.href ? "active" : ""} aria-current={pathname === item.href ? "page" : undefined} href={item.href} onClick={() => setMobile(false)}><span aria-hidden="true">{item.icon}</span>{item.label}</Link>)}</section>)}</nav><div className="repo-card"><small>CONNECTED CONTEXT</small><b>dev-life / workspace</b><span><i /> Read-only preview</span></div></aside>
    {mobile && <button className="scrim" aria-label="ปิดเมนู" onClick={() => setMobile(false)} />}
    <section className="workspace"><header className="topbar"><button className="mobile-toggle" aria-label="เปิดเมนู" aria-expanded={mobile} aria-controls="workspace-navigation" onClick={() => setMobile(true)}>Menu</button><div className="project-crumb"><span>Projects</span><b>/ {projectName || "Create project"}</b></div><button className="search-trigger" aria-haspopup="dialog" onClick={() => { setActive(0); setPalette(true); }}><span>Search or jump to…</span><kbd>⌘ K</kbd></button><div className="top-actions"><Link href="/checklists">Add task</Link><Dropdown label={<span className="account-trigger">{displayName?.slice(0, 1).toUpperCase() || email?.slice(0, 1).toUpperCase() || "U"}</span>}><div className="account-menu">{displayName && <b>{displayName}</b>}<span>{email}</span><Link href="/settings">ตั้งค่าบัญชี</Link><form action={logout}><button type="submit">Log out</button></form></div></Dropdown></div></header><div className="content">{children}</div></section>
